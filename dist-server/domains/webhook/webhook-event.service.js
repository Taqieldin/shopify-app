...
export class WebhookEventService {
     ...
     */
    static async createWebhook(shopId, dto, actorId) {
     ...
     */
    static async sendWebhook(shopId, event, data) {
        ...
        await Promise.allSettled(subscribedWebhooks.map((webhook) => this.deliverWebhook(webhook, event, data)));
    }
    /**
     ...
     */
    static async handleWebhookFailure(webhookId, event, payload, response) {
        await prisma.webhookEndpoint.update({
            where: { id: webhookId },
     ...
     */
    static async handleWebhookError(webhookId, event, payload, error) {
        await prisma.webhookEndpoint.update({
            where: { id: webhookId },
            data: { failure_count: { increment: 1 } },
        });
     ...
     */
    static async listWebhooks(shopId) {
        const webhooks = await prisma.webhookEndpoint.findMany({
     ...
     * Delete a webhook
     */
    static async deleteWebhook(shopId, webhookId, actorId) {
     ...
     */
    static async getDeliveryHistory(shopId, webhookId, limit = 50) {
        const webhook = await prisma.webhookEndpoint.findFirst({
            where: { id: webhookId, shop_id: shopId },
        });
        if (!webhook) {
 ...
 */
export async function triggerWebhook(shopId, event, data) {
