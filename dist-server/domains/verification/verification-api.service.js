...
export class VerificationAPIService {
     ...
     */
    static async verifyBySerial(shopId, serial) {
     ...
     * Verify by NFC UID
     */
    static async verifyByNFC(shopId, nfcUid) {
     ...
     */
    static async quickStatus(shopId, serial) {
        const piece = await prisma.physicalPiece.findUnique({
            where: { shop_id_serial: { shop_id: shopId, serial } },
            include: {
                lost_reports: { where: { status: 'CONFIRMED' } },
     ...
     */
    static async batchVerify(shopId, serials) {
