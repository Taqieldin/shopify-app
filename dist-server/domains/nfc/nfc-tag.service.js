...
export class NFCTagService {
     ...
     */
    static async registerTag(shopId, dto, actorId) {
        ...
        if (existingNFC) {
            throw new ConflictError(`NFC tag UID '${dto.nfc_uid}' is already assigned to another piece (${existingNFC.serial}).`);
        }
     ...
     */
    static async updateTag(shopId, pieceId, dto, actorId) {
        const piece = await prisma.physicalPiece.findFirst({
        ...
        if (dto.nfc_uid && dto.nfc_uid !== piece.nfc_uid) {
            ...
            if (existingNFC) {
                throw new ConflictError(`NFC tag UID '${dto.nfc_uid}' is already assigned to another piece.`);
            }
        }
        const updated = await prisma.physicalPiece.update({
            where: { id: pieceId },
     ...
     */
    static async unregisterTag(shopId, pieceId, actorId) {
        const piece = await prisma.physicalPiece.findFirst({
            where: { id: pieceId, shop_id: shopId },
        });
        if (!piece) {
     ...
     */
    static async lookupByUID(shopId, nfcUid) {
        const piece = await prisma.physicalPiece.findFirst({
            where: {
                shop_id: shopId,
                nfc_uid: nfcUid,
     ...
     */
    static async listTagged(shopId) {
     ...
     */
    static async writeOwnerMetadata(shopId, pieceId, metadata) {
        const piece = await prisma.physicalPiece.findFirst({
            where: { id: pieceId, shop_id: shopId },
            include: {
        ...
        const payload = {
            ...
            previous_owner_shopify_customer_id: metadata.previous_owner_shopify_customer_id ||
     ...
     */
    static async listWriteLogs(shopId, pieceId) {
     ...
     */
    static async bulkRegister(shopId, records, actorId) {
        const results = {
            success: [],
            failed: [],
        };
