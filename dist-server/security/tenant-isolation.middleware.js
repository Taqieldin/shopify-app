 ...
 */
export async function validateTenantAccess(shopId, resourceType, resourceId) {
    // Map resource types to their models
    const validators = {
        physical_piece: () => prisma.physicalPiece.findFirst({
            where: { id: resourceId, shop_id: shopId },
            select: { id: true },
        }),
        passport: () => prisma.passport.findFirst({
            where: { id: resourceId, shop_id: shopId },
            select: { id: true },
        }),
        ownership: () => prisma.ownership.findFirst({
            where: { id: resourceId, shop_id: shopId },
            select: { id: true },
        }),
        ...
        service_case: () => prisma.serviceCase.findFirst({
            where: { id: resourceId, shop_id: shopId },
            select: { id: true },
        }),
        api_key: () => prisma.aPIKey.findFirst({
            where: { id: resourceId, shop_id: shopId },
            select: { id: true },
        }),
        webhook: () => prisma.webhookEndpoint.findFirst({
            where: { id: resourceId, shop_id: shopId },
            select: { id: true },
        }),
    };
 ...
 */
export async function auditTenantAccess(shopId, actorId, resourceType, resourceId, action, success) {
 ...
 */
export function validateShopIdParam(req, res, next) {
 ...
 */
export function createTenantIsolationMiddleware(currentShopId) {
