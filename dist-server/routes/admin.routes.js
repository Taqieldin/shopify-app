 ...
 */
adminRouter.get('/passports', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/passports', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/pieces', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/pieces', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/credits', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/theft', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/csv-import', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/billing', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/billing/upgrade', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/nfc', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/nfc/register', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.patch('/nfc/:pieceId', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const result = await NFCTagService.updateTag(shopId, req.params.pieceId, req.body, req.actorId || 'admin');
        res.json({ success: true, data: result });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.delete('/nfc/:pieceId', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const result = await NFCTagService.unregisterTag(shopId, req.params.pieceId, req.actorId || 'admin');
        res.json({ success: true, data: result });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.post('/nfc/bulk', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/qr/:serial/png', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/qr/:serial/svg', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 */
adminRouter.get('/qr/:serial', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 */
adminRouter.get('/labels/:serial', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 */
adminRouter.get('/labels/:serial/download', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
        const data = await LabelGeneratorService.getLabelData(shopId, serial);
 ...
 */
adminRouter.get('/labels/batch', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serials = req.query.serials?.split(',').filter(Boolean) || [];
        if (serials.length === 0) {
 ...
 */
adminRouter.get('/certificates/:serial', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 */
adminRouter.get('/certificates', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/provenance/:serial', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 */
adminRouter.get('/provenance/:serial/public', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 */
adminRouter.get('/certificates/:serial/pdf', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const serial = req.params.serial;
 ...
 * Update API key
 */
adminRouter.patch('/api-keys/:keyId', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { APIKeyService } = await import('../domains/api-key/api-key.service.js');
        const updated = await APIKeyService.updateAPIKey(shopId, req.params.keyId, req.body, req.actorId || 'admin');
        res.json({ success: true, data: updated });
    }
    catch (err) {
        next(err);
 ...
 * Revoke API key
 */
adminRouter.delete('/api-keys/:keyId', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { APIKeyService } = await import('../domains/api-key/api-key.service.js');
        await APIKeyService.revokeAPIKey(shopId, req.params.keyId, req.actorId || 'admin');
 ...
 */
adminRouter.get('/api-keys/:keyId/stats', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { APIKeyService } = await import('../domains/api-key/api-key.service.js');
        const stats = await APIKeyService.getKeyStats(shopId, req.params.keyId);
 ...
 * Delete webhook
 */
adminRouter.delete('/webhooks/:webhookId', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { WebhookEventService } = await import('../domains/webhook/webhook-event.service.js');
        await WebhookEventService.deleteWebhook(shopId, req.params.webhookId, req.actorId || 'admin');
        res.json({ success: true });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.get('/webhooks/:webhookId/deliveries', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const limit = req.query.limit ? parseInt(req.query.limit) : 50;
        const { WebhookEventService } = await import('../domains/webhook/webhook-event.service.js');
        const deliveries = await WebhookEventService.getDeliveryHistory(shopId, req.params.webhookId, limit);
 ...
 */
adminRouter.get('/security/alerts', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/security/alerts/:eventId/acknowledge', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { SecurityMonitorService } = await import('../security/security-monitor.service.js');
        await SecurityMonitorService.acknowledgeAlert(shopId, req.params.eventId, req.actorId || 'admin', req.body.notes);
        res.json({ success: true });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.post('/security/block-nfc', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { SecurityMonitorService } = await import('../security/security-monitor.service.js');
        ...
        await SecurityMonitorService.blockNFCUID(shopId, nfc_uid, reason, req.actorId || 'admin');
 ...
 */
adminRouter.get('/security/scan-patterns/:serial', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const { SecurityMonitorService } = await import('../security/security-monitor.service.js');
        const pattern = await SecurityMonitorService.getScanPattern(shopId, req.params.serial);
        if (!pattern) {
 ...
 */
adminRouter.get('/security/blocked-nfc', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 * Unblock an NFC UID
 */
adminRouter.delete('/security/blocked-nfc/:nfcUid', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        await prisma.nFCBlocklist.updateMany({
            where: {
                shop_id: shopId,
                nfc_uid: req.params.nfcUid,
            },
            data: {
                is_active: false,
 ...
 */
adminRouter.get('/verification-options', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.patch('/verification-options/:productId', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const result = await VerificationOptionsService.setMethods(shopId, req.params.productId, req.body.methods || [], req.actorId || 'admin');
        res.json({ success: true, data: result });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.get('/resale', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.post('/resale/:listingId/cancel', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const result = await ResaleService.adminCancelListing(shopId, req.params.listingId, req.actorId || 'admin');
        res.json({ success: true, data: result });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.patch('/events/:eventId/status', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const event = await ClubEventService.setStatus(shopId, req.params.eventId, req.body.status, req.actorId || 'admin');
        res.json({ success: true, data: event });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.post('/events/:eventId/check-in', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
 ...
 */
adminRouter.get('/nfc/writes', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
        const logs = await NFCTagService.listWriteLogs(shopId, req.query.pieceId);
        res.json({ success: true, data: logs });
    }
    catch (err) {
        next(err);
 ...
 */
adminRouter.get('/credits', async (req, res, next) => {
    try {
        const shopId = req.tenant.shop_id;
