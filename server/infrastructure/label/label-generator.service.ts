...
export class LabelGeneratorService {
   ...
   */
  static async getLabelData(shopId: string, serial: string, options?: LabelOptions): Promise<LabelData> {
   ...
   */
  static generateLabelHTML(data: LabelData, options?: LabelOptions): string {
...
<body>
   ...
   */
  static async generateSVGLabel(shopId: string, serial: string, options?: LabelOptions): Promise<string> {
    const data = await this.getLabelData(shopId, serial, options);
    const width = options?.width || 300;
    const height = options?.height || 150;
