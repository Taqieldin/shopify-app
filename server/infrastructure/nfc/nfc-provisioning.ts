...
/**
 * NFC Tag Types supported by the provisioning engine
 */
export type NFCTagType = 'NTAG_424_DNA' | 'NTAG_213' | 'NTAG_215' | 'NTAG_216' | 'ICODE_SLIX';

export interface NFCProvisioningPayload {
  tag_type: NFCTagType;
  serial: string;
  shop_domain: string;
  nfc_uid: string;
  ndef_url: string;
  rolling_counter_enabled: boolean;
  sun_authentication: boolean;
  provisioning_commands: NFCCommand[];
  binding_hash: string;
...
export interface ProvisionTagDTO {
  serial: string;
  shop_domain: string;
  nfc_uid: string;
  tag_type?: NFCTagType;
...
export class NFCProvisioningEngine {
  private static DEFAULT_BASE_URL = process.env.PUBLIC_APP_URL || 'https://passport.app';
   ...
   */
  static generateProvisioningPayload(dto: ProvisionTagDTO): NFCProvisioningPayload {
   ...
   */
  private static generateNTAG424Commands(dto: ProvisionTagDTO, ndefUrl: string, bindingHash: string): NFCCommand[] {
   ...
   */
  private static generateNTAGClassicCommands(dto: ProvisionTagDTO, ndefUrl: string, tagType: NFCTagType): NFCCommand[] {
  ...
  static generateBatchPayloads(
    shop_domain: string,
    items: Array<{ serial: string; nfc_uid: string; tag_type?: NFCTagType }>,
    passport_base_url?: string
  ): NFCProvisioningPayload[] {
    return items.map((item) =>
      this.generateProvisioningPayload({
        serial: item.serial,
        shop_domain,
