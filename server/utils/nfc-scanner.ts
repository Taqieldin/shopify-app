 ...
 */
export function formatNFCUID(uid: string): string {
 ...
 */
export function parseNDEFMessage(message: any): {
  type: string;
  data: string;
  encoding?: string;
} | null {
 ...
 */
export function generateNFCTagURL(baseUrl: string, serial: string): string {
 ...
 */
export function detectNTAGType(uid: string): string {
  const normalized = normalizeNFCUID(uid);
...
/**
 * Security check: Detect potentially cloned tags
 * (This is a basic check - production should use cryptographic verification)
 */
...
export function performBasicSecurityCheck(
  uid: string,
  previousScans?: Array<{ timestamp: Date; location?: string }>
): SecurityCheck {
  ...
  if (previousScans && previousScans.length > 10) {
    const lastHour = previousScans.filter(
      (scan) => Date.now() - scan.timestamp.getTime() < 60 * 60 * 1000
    );
