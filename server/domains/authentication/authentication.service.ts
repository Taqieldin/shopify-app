...
export class AuthenticationService {
   ...
   */
  static async verifyPiece(shop_id: string, dto: VerifyAuthenticationDTO) {
    ...
    if (dto.nfc_uid) {
      ...
      const securityAlerts = await SecurityMonitorService.analyzeScan(
        shop_id,
        dto.serial,
        dto.nfc_uid,
        {
    ...
    let result: 'AUTHENTICATED' | 'UNREGISTERED' | 'PENDING_VERIFICATION' | 'SUSPICIOUS' | 'REVOKED' = 'AUTHENTICATED';
   ...
   */
  private static async assertMethodEnabled(
    shop_id: string,
    productRef: { id: string; verification_methods: string | null },
    method: VerificationMethod
  ) {
    ...
    let configured: VerificationMethod[] = [];
    ...
    const shopAllowed: VerificationMethod[] = [];
    if (shopFlags?.nfc_enabled !== false) shopAllowed.push('NFC');
    if (shopFlags?.qr_enabled !== false) shopAllowed.push('QR');
    ...
    if (!effective.includes(method)) {
      throw new ConflictError(
        `${method} verification is not enabled for this product. Enabled methods: ${effective.join(', ') || 'none'}.`
      );
    }
  }
   ...
   */
  static async getAdminAuthLogs(shop_id: string, options: { limit?: number; offset?: number } = {}) {
   ...
   */
  static async getRiskEvents(shop_id: string) {
