...
export interface RiskAnalysisResult {
  ...
  anomalies: Array<{
    type: 'IMPOSSIBLE_TRAVEL' | 'RAPID_VELOCITY' | 'COUNTER_MISMATCH' | 'REPEATED_FAILURES' | 'BLACKLIST_HIT';
...
export class RiskEngine {
  ...
  static evaluate(
    ...
    currentScan: ScanTelemetry,
    isReportedStolen = false
  ): RiskAnalysisResult {
    const anomalies: RiskAnalysisResult['anomalies'] = [];
    ...
    if (recentScans.length > 0) {
      ...
      const scansInLastTwoMinutes = recentScans.filter(
        (s) => (currentScan.timestamp.getTime() - s.created_at.getTime()) / 1000 < 120
      );
      ...
      if (
        currentScan.nfc_read_counter != null &&
    ...
    let risk_level: RiskAnalysisResult['risk_level'] = 'NORMAL';
