...
export interface CreateEarlyAccessDTO {
...
export class EarlyAccessService {
   ...
   */
  static async createRule(shop_id: string, dto: CreateEarlyAccessDTO) {
   ...
   */
  static async evaluateCustomerAccess(
    shop_id: string,
    shopify_product_id: string,
    ...
    const activeRule = rules.find(
      (r) => now >= new Date(r.starts_at) && (!r.ends_at || now <= new Date(r.ends_at))
    );
