...
export interface ListForResaleDTO {
  serial: string;
  seller_shopify_customer_id: string;
  price: number;
  currency?: string;
  notes?: string;
}

export interface BuyResaleDTO {
  listing_id: string;
  buyer_shopify_customer_id: string;
  buyer_email: string;
  buyer_name?: string;
}
...
export class ResaleService {
   ...
   */
  static async listPiece(shop_id: string, dto: ListForResaleDTO) {
   ...
   */
  static async getListings(shop_id: string, status = 'LISTED') {
   ...
   */
  static async cancelListing(shop_id: string, listing_id: string, seller_shopify_customer_id: string) {
   ...
   */
  static async buyListing(shop_id: string, dto: BuyResaleDTO) {
    const listing = await prisma.resaleListing.findFirst({
   ...
   */
  static async adminCancelListing(shop_id: string, listing_id: string, actorId: string) {
    const listing = await prisma.resaleListing.findFirst({
      where: { id: listing_id, shop_id },
    });
   ...
   */
  static async listAllForAdmin(shop_id: string) {
