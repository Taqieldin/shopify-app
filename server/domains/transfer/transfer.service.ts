...
export interface InitiateTransferDTO {
  serial: string;
  sender_shopify_customer_id: string;
...
export class TransferService {
   ...
   */
  static async initiateTransfer(shop_id: string, dto: InitiateTransferDTO) {
   ...
   */
  static async getTransferByToken(shop_id: string, transfer_token: string) {
   ...
   */
  static async acceptTransfer(shop_id: string, dto: AcceptTransferDTO) {
    const transfer = await prisma.ownershipTransfer.findUnique({
    ...
    TransferStateMachine.validateTransition(transfer.status as TransferStatus, 'ACCEPTED');
   ...
   */
  static async cancelTransfer(shop_id: string, transfer_token: string, sender_shopify_customer_id: string) {
    const transfer = await prisma.ownershipTransfer.findUnique({
      where: { transfer_token },
      include: { sender: true },
    });
    ...
    TransferStateMachine.validateTransition(transfer.status as TransferStatus, 'CANCELLED');
