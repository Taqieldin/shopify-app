 ...
 * Self-service API keys for third-party integrations
 */
...
export interface CreateAPIKeyDTO {
  ...
  scopes?: string[]; // e.g., ['verify:read', 'provenance:read']
...
export class APIKeyService {
   ...
   */
  static async verifyAPIKey(key: string): Promise<APIKeyData | null> {
    const hashedKey = this.hashKey(key);
   ...
   */
  static async listAPIKeys(shopId: string) {
   ...
   * Revoke an API key
   */
  static async revokeAPIKey(shopId: string, keyId: string, actorId: string) {
    const apiKey = await prisma.aPIKey.findFirst({
  ...
  static async updateAPIKey(
    shopId: string,
    keyId: string,
    updates: { name?: string; tier?: string; rateLimit?: number; scopes?: string[] },
    actorId: string
  ) {
    const apiKey = await prisma.aPIKey.findFirst({
      where: { id: keyId, shop_id: shopId },
   ...
   */
  static async getKeyStats(shopId: string, keyId: string) {
    const apiKey = await prisma.aPIKey.findFirst({
      where: { id: keyId, shop_id: shopId },
    });
   ...
   */
  static hasScope(keyData: APIKeyData, requiredScope: string): boolean {
    // Wildcard scope
   ...
   */
  private static formatKeyData(apiKey: any): APIKeyData {
    return {
      id: apiKey.id,
 ...
 */
export async function authenticateAPIKey(key: string): Promise<APIKeyData> {
