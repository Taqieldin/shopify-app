...
import { LedgerTransactionType } from '../../shared/types/index.js';

export interface PostCreditTransactionDTO {
...
export class CreditsService {
   ...
   */
  static async postTransaction(shop_id: string, dto: PostCreditTransactionDTO, actor_id: string) {
   ...
   */
  static async getBalance(shop_id: string, customer_shopify_id: string): Promise<number> {
    const customer = await prisma.customer.findUnique({
      where: {
        shop_id_shopify_customer_id: {
          shop_id,
   ...
   */
  static async getStatement(shop_id: string, customer_shopify_id: string, limit = 50) {
    const customer = await prisma.customer.findUnique({
      where: {
        shop_id_shopify_customer_id: {
          shop_id,
   ...
   */
  static async listAdminTransactions(shop_id: string, limit = 50, offset = 0) {
